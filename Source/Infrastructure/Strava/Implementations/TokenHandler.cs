...
internal sealed class TokenHandler(ITokenStore tokenStore, IOptions<StravaOptions> options) : DelegatingHandler
{
    private readonly string _authScheme = options.Value.AuthScheme;

    protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
    {
        string authToken = await tokenStore.GetTokenAsync(cancellationToken);
